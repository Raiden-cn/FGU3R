...
class DataProcessor(object):
    ...
    def transform_points_to_voxels_mm(self, data_dict=None, config=None, voxel_generator=None):
        if data_dict is None:
            ...
            # just bind the config, we will create the VoxelGeneratorWrapper later,
            # to avoid pickling issues in multiprocess spawn
            return partial(self.transform_points_to_voxels_mm, config=config)
        ...
        if self.voxel_generator is None:
            ...
            self.voxel_generator_mm = VoxelGeneratorWrapper(
                vsize_xyz=config.VOXEL_SIZE,
                coors_range_xyz=self.point_cloud_range,
        ...
        if self.skip_voxel_generator:
            ...
            if 'points_pseudo' in data_dict:
                points = data_dict['points_pseudo']
                voxel_output = self.voxel_generator_mm.generate(points)
                voxels, coordinates, num_points = voxel_output
                if not data_dict['use_lead_xyz_pseudo']:
                    voxels = voxels[..., 3:]  # remove xyz in voxels(N, 3)
                data_dict['voxels_pseudo'] = voxels
                data_dict['voxel_coords_pseudo'] = coordinates
                data_dict['voxel_num_points_pseudo'] = num_points

...
class VoxelSetAbstraction(nn.Module):
    def __init__(self, model_cfg, voxel_size, point_cloud_range, num_bev_features=None,
                 num_rawpoint_features=None, num_pseudopoint_features=None, **kwargs):
        super().__init__()
        ...
        self.use_pseudo_uv = self.model_cfg.get('USE_PSEUDO_UV', True)
        if not self.use_pseudo_uv:
            num_pseudopoint_features -= 2
        ...
        for src_name in self.model_cfg.FEATURES_SOURCE:
            if src_name in ['bev', 'raw_points', 'pseudo_points']:
                continue
            if src_name in ['x_conv1_mm', 'x_conv2_mm', 'x_conv3_mm', 'x_conv4_mm']:
        ...
        if 'pseudo_points' in self.model_cfg.FEATURES_SOURCE:
            self.SA_pseudopoints, cur_num_c_out = pointnet2_stack_modules.build_local_aggregation_module(
                input_channels=num_pseudopoint_features - 3 ,
                config=SA_cfg['pseudo_points']
            )
            c_in += cur_num_c_out
        ...
        self.num_point_features.update({'x_conv3' : 64,
                                        'x_conv4' : 64,
                                        'PFE': self.model_cfg.NUM_OUTPUT_FEATURES
                                       })
    ...
    def interpolate_from_bev_features(self, keypoints, bev_features, batch_size, bev_stride):
        """
        Args:
            keypoints: (N1 + N2 + ..., 4) [8192, 4]
            bev_features: (B, C, H, W) [4, 256, 200, 176]
            batch_size:
            bev_stride:
    ...
    def aggregate_keypoint_features_from_one_source(
        ...
        Args:
            aggregate_func:
            xyz: (N, 3)  原始点云坐标
            xyz_features: (N, C) 原始点云features
            xyz_bs_idxs: (N) 原始点云batch索引
            new_xyz: (M, 3) 关键点坐标
            new_xyz_batch_cnt: (batch_size), [N1, N2, ...] 关键点个数 per bs
    ...
    def forward(self, batch_dict):
        ...
        for k in range(batch_size):
            new_xyz_batch_cnt[k] = (keypoints[:, 0] == k).sum() # 关键点个数per sample
        ...
        if 'pseudo_points' in self.model_cfg.FEATURES_SOURCE:
            pseudo_points = batch_dict['points_pseudo']
            pseudo_points[:, 4:7] /= 255.0 # rgb 归一化
            if not self.use_pseudo_uv:
                pseudo_points = pseudo_points[:, :7]
            pooled_features = self.aggregate_keypoint_features_from_one_source(
                batch_size=batch_size, aggregate_func=self.SA_pseudopoints,
                xyz=pseudo_points[:, 1:4],
                xyz_features=pseudo_points[:, 4:].contiguous() if pseudo_points.shape[1] > 4 else None,
                xyz_bs_idxs=pseudo_points[:, 0],
                new_xyz=new_xyz, new_xyz_batch_cnt=new_xyz_batch_cnt,
                filter_neighbors_with_roi=self.model_cfg.SA_LAYER['pseudo_points'].get('FILTER_NEIGHBOR_WITH_ROI', False),
                radius_of_neighbor=self.model_cfg.SA_LAYER['pseudo_points'].get('RADIUS_OF_NEIGHBOR_WITH_ROI', None),
                rois=batch_dict.get('rois', None)
            )
            point_features_list.append(pooled_features)
        ...
        for k, src_name in enumerate(self.SA_layer_names):
            ...
            if "mm" in src_name:
                src_name_mm = src_name[:-3]
                cur_coords = batch_dict['multi_scale_3d_features_mm'][src_name_mm].indices
                cur_features = batch_dict['multi_scale_3d_features_mm'][src_name_mm].features.contiguous()
            else:
